"""In-memory mock for the long-term memory store.

Production memory uses a vector DB (Qdrant / Pinecone / Weaviate) — see
``docs/modules/memory.md``. The mock stores items in a process-local dict
and provides a naive substring-based search so the rest of the platform
can be wired up and tested without external dependencies.
"""
